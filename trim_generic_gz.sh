...
#MIDDLE=_sequence_
ENDING="_sequence.txt.gz|.fq.gz|.txt.gz|.fq|.txt"

if [[ $1 == '-h' || $1 == '--help' ]]; then
    echo "usage: $0 BASE_NAMES"
    echo "goes through subdirs and creates trimmed files for gzipped read file names"
...
for i in $BASE_NAMES; do
 ...
 #   BASE_NAME=${FILE[0]%_[12]+(${ENDING})}
	...
	echo nohup time perl ${POPO}/basic-pipeline/trim-fastq_gz.pl --input1 ${DIR}/${FILE[0]} --input2 ${DIR}/${FILE[1]} --output ${BASE_NAME}_trimmed --quality-threshold 18 --fastq-type $FQ_TYPE --min-length 50 --no-5p-trim >>  ${DIR}/${BASE_NAME}.log 
	nohup time perl ${POPO}/basic-pipeline/trim-fastq_gz.pl --input1 ${DIR}/${FILE[0]} --input2 ${DIR}/${FILE[1]} --output ${BASE_NAME}_trimmed --quality-threshold 18 --fastq-type $FQ_TYPE --min-length 50 --no-5p-trim  >> ${DIR}/${BASE_NAME}_trimstats.log 2>> ${DIR}/${BASE_NAME}.log & 
    fi
    cd $BASE_DIR
done
