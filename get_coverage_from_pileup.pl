#!/usr/bin/perl
# author: Lukas Endler Mon 25 November 2013
# takes an inputstream in mpileup format and calculates average coverage and stdv chromosome wise and overall for each population
# useage: perl get_coverage_from_pileup.pl < infile.mpileup > coverage.txt
use warnings;
use strict;
#use Data::Dumper;
#use Getopt::Long;
...
if (-t STDIN and not @ARGV) {
  die "useage: perl get_coverage_from_pileup.pl < infile.mpileup > coverage.txt\ntakes an inputstream in mpileup format and calculates average coverage and stdv chromosome wise and overall for each population\n";  
} 
...
while (<>){
  ...
  unless ($chroms{$c_name}{'Length'} % 1e6) {print STDERR "Chrom $c_name : ",$chroms{$c_name}{'Length'},"\n"}; 
  unless ($pops) {$pops = ($#fields-2)/3 -1};
  for my $i (0..$pops) {
    my $val = $fields[($i*3)+3];
    #print "$val $i $pops\n";
    unless (exists $chroms{$c_name}{'Sum'}[$i]) {
      for (qw( Sum SSum Zeros)) {${$chroms{$c_name}{$_}}[$i] = 0}; 
      #push(@{$chroms{$c_name}{'Sum'}}, 0);
    }
    $chroms{$c_name}{'Sum'}[$i] += $val;
    $chroms{$c_name}{'SSum'}[$i] += $val*$val;
  }
 # ($count == 1000) && last;
} 
