...
def get_cat_counts(sig_snpsA,snp_dict):
    cat_counts= 0
    for chrom in sig_snpsA.keys():
        for bps in sig_snpsA[chrom]['bps']:
            idx = snp_dict[chrom]['bps'].searchsorted(bps)
            cat_counts += snp_dictA[chrom]['cat'][idx]
    return cat_counts
    
def add_cat(snp_dict,cat_file):
    """
    reads a sync file with category added as last column (0,1)
    """
    if re.search("\.b?gz",cat_file):
        inf = gzip.open(cat_file,'rb')
    else:
        inf = open(cat_file,"r")
    #inf = open(ap_file,"r")
    for i in snp_dict.keys():
        snp_dict[i]['cat']=np.empty(len(snp_dict[i]['bps']), dtype=int)
    for line in inf:
        if re.match("\#",line):
            continue
        line.rstrip()
        ...
        if fields[0] in snp_dict.keys():
            ...
            if (idx < len(snp_dict[fields[0]]['bps'])) and snp_dict[fields[0]]['bps'][idx] == bps:
                snp_dict[fields[0]]['cat'][idx]= int(fields[-1])
    return True
...
def get_random_cats(snp_dict,sig_snp,chroms):
    """
    return a dictionary of chromosomes with a numpy array of two entries (not category and category)
    """
    cats = { x : np.zeros(2,dtype=int) for x in chroms }
    cats['total']= np.zeros(2,dtype=int)
    for chrom in chroms:
        if len(sig_snp[chrom]['bps']) < 1:
            continue
        idxs = create_random_sample_alt(snp_dict[chrom]['bin_idx'],sig_snp[chrom]['afs_hist']['counts'])
        for idx in idxs:
            if snp_dict[chrom]['cat'][idx]:
                # add to the right field
                cats[chrom][snp_dict[chrom]['cat'][idx] ] += 1
                cats['total'][snp_dict[chrom]['cat'][idx] ] += 1 
    return cats
...
parser.add_argument("--aA", dest="aA", help="syncfile A with category", required=True)
...
if verb:
    print "adding category data from "+ aA
add_cat(snp_dictA,aA)
...
sig_snp_num['total']=np.sum([ sig_snp_num[x] for x in sig_snpsA.keys()])
#get category counts for sig snps
sig_snp_num['cats']=get_cat_counts(sig_snpsA,snp_dictA)
# create histogram
if verb:
...
# create dict with category counts
cat_counter= { x: np.zeros((rnd_samp,2),dtype=int) for x in chroms }
cat_counter['total']= np.zeros((rnd_samp,2),dtype=int)
...
for i in range(0,rnd_samp):
    if verb and i%10000 == 0:
        print "sample " + str(i)
    rnd_cats=get_random_cats(snp_dictA,sig_snpsA,chroms)
    for chrom in rnd_cats.keys():
        cat_counter[chrom][i] = rnd_cats[chrom]
# write out results as histogram file
out_str="\t".join([ x+str(y) for x in ["C_"]  for y in quant ])
out_str = "Chrom\t"+out_str
with open(outfile,'w') as of:
    if verb:
        print "writing outputfile" + outfile
    print >>of, "\# "+str(rnd_samp)+" random samples, sample size: "+str(sig_snp_num['all_chroms'])+"; snps on sampled chroms:"+ str(sig_snp_num['total']) + "; cat counts in sign snps: " + str(sig_snp_num['cats'])
    print >>of, out_str
    # total = False
    chrom='total'
    cat_tots=np.array([ x[1] for x in cat_counter[chrom] ])
    output="\t".join([ str(stats.scoreatpercentile(cat_tots,y)) for y in quant ])
    print >>of, "{}\t{}".format(chrom,output)
    print >>of, "category counts lower than sig snps:\t"+str(len(cat_tots[cat_tots < sig_snp_num['cats'] ]))  
