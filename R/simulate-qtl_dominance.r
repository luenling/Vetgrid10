...
get.V.E <- function(afs_loc,eff_loc,h_loc,h2=0.5,F.in=0,n=1000,eps.int=0.0){
    ...
    ## interaction component, roughly centered around 0
...
get.V.E2 <- function(afs_loc,eff_loc,h_loc,h2=0.5,F.in=0,n=1000,eps.int=0.0){
  gt.freqs = sapply(afs_loc,function (p) c(p^2 + (p-p^2)*F.in,2*(p-p^2)*(1-F.in),(1-p)^2 + (p-p^2)*F.in))
  pos <- c()
  pos <- apply(gt.freqs,2,function (x) get.gt.locus(x,c(2,1,0),n))
  pos <- matrix(pos,nrow=length(afs_loc),ncol=n,byrow=T)
...
gt.eff=apply(gt.ab*2,1,function(x){(loc.eff(h.eps,x))})
gt.eff.ind=gt.eff-0.5

plot(gt.a,(eff.a*gt.eff.ind[1,1:3]+eff.b*gt.eff.ind[2,1:3]),type="n",col="red",ylim=c(-1,1), main="Independent Loci",xaxt="n",xlab="Genotype",ylab="Phenotypic effect")
for (i in 1:3){
    idxs=(3*(i-1)+1):(3*i)
    lines(gt.a,(eff.a*gt.eff.ind[1,idxs]+eff.b*gt.eff.ind[2,idxs]),type="o",col=colors[i])
}
...
quartz(width=5,height=5)
par(mar=c(4,4,2.5,0.5))
plot(gt.a,(eff.a*gt.a+eff.b*gt.b[1]),type="n",ylim=c(-1.5,1.5), main="Positively Interacting Loci",xaxt="n",xlab="Genotype",ylab="Phenotypic effect")
for (i in 1:3){
    idxs=(3*(i-1)+1):(3*i)
    #lines(gt.a,(eff.a*gt.eff.ind[1,idxs]+eff.b*gt.eff.ind[2,idxs] + (eff.b*gt.eff[1,idxs]*gt.eff[2,idxs] - 1/2)),type="o",lty=1,col=colors[i])
    lines(gt.a,(eff.a*gt.eff.ind[1,idxs]+eff.b*gt.eff.ind[2,idxs] + (eff.b*gt.eff.ind[1,idxs]*gt.eff.ind[2,idxs])),type="o",lty=1,col=colors[i])
}
axis(1,at=c(0,0.5,1),labels=c("aa","aA","AA"))
legend("bottomright",legend=c("bb","bB","BB"),col=colors,lty=1,pch=1)
...
quartz(width=5,height=5)
par(mar=c(4,4,2.5,0.5))
plot(gt.a,(eff.a*gt.a+eff.b*gt.b[1]),type="n",ylim=c(-1.5,1.5), main="Negatively Interacting Loci",xaxt="n",xlab="Genotype",ylab="Phenotypic effect")
for (i in 1:3){
    idxs=(3*(i-1)+1):(3*i)
    #lines(gt.a,(eff.a*gt.eff.ind[1,idxs]+eff.b*gt.eff.ind[2,idxs] - (eff.b*gt.eff[1,idxs]*gt.eff[2,idxs] - 1/2)),type="o",lty=1,col=colors[i])
  lines(gt.a,(eff.a*gt.eff.ind[1,idxs]+eff.b*gt.eff.ind[2,idxs] - (eff.b*gt.eff.ind[1,idxs]*gt.eff.ind[2,idxs] )),type="o",lty=1,col=colors[i])
...
#### if random.cov is set to a number, a poisson distr random coverage around that number is chosen and the allelel counts are sampled from a binomial
#######  Initialize
     d.replicates=total.ind/n.replicates # ind. per replicate
...
### alleles A a
    ...
    ret.list[["h.loci"]]=matrix(unlist(h.loci.loc),nrow=length(h.loci.loc),byrow = T)
    ret.list[["freqs.loci"]]=matrix(unlist(n.freqs.loc),nrow=length(n.freqs.loc),byrow = T)
...
eff.mat <- matrix(rep.int(eff,n.runs),nrow=n.runs,byrow = T)
d.mat <- eff.mat * (res.afs.h$h.loci - 0.5) * 2
p.mat <- res.afs.h$freqs.loci
q.mat <- 1 - res.afs.h$freqs.loci
all.eff= -p.mat *( eff + (q.mat - p.mat) * d.mat)
alpha.eff= eff + (q.mat - p.mat) * d.mat
...
res.afs.h$sp.lor.alpha=sapply(1:n.runs,function (x) -1*(cor(res.afs.h$lor[x,],alpha.eff[x,],method="spearman")))
res.afs.h$p.lor.alpha=sapply(1:n.runs,function (x) -1*(cor(res.afs.h$lor[x,],alpha.eff[x,],method="pearson")))
res.afs.h$sp.pv.alpha=sapply(1:n.runs,function (x) cor(res.afs.h$lpv[x,],abs(alpha.eff[x,]),method="spearman"))
res.afs.h$p.pv.alpha=sapply(1:n.runs,function (x) cor(res.afs.h$lpv[x,],abs(alpha.eff[x,]),method="pearson"))

summary(res.afs.h$sp.lor)
summary(res.afs.h$p.lor)
summary(res.afs.h$sp.lor.alpha)
summary(res.afs.h$p.lor.alpha)
plot(p.mat[,17],res.afs.h$lor[,17])
abline( lm(res.afs.h$lor[,17] ~ p.mat[,17]))

plot.trend <- function(x,y, ylab="lOR", main = "", ... ){
  main=paste(main,"cor=",cor(x,y))
  plot(x,y, ylab=ylab, main = main , ...)
  abline(lm(y~x))
  return(cor(x,y))
}
plot.trend(d.mat[p.mat[,17] == 0.8,17],res.afs.h$lor[p.mat[,17] == 0.8,17],ylab="lOR",xlab="d", main="p = 0.8")
plot.trend(res.afs.h$h.loci[p.mat[,17] == 0.8,17],res.afs.h$lor[p.mat[,17] == 0.8,17],ylab="lOR",xlab="h",main= "p = 0.8")
plot.trend(res.afs.h$h.loci[p.mat[,17] == 0.2,17],res.afs.h$lor[p.mat[,17] == 0.2,17], xlab="h",main= "p = 0.2")
plot.trend(alpha.eff[p.mat[,17] == 0.8,17],res.afs.h$lor[p.mat[,17] == 0.8,17],xlab="alpha",main= "p = 0.8")
plot.trend(alpha.eff[p.mat[,17] >= 0.5,17],res.afs.h$lor[p.mat[,17] >= 0.5,17],xlab="alpha",main= "p >= 0.5")
plot.trend(alpha.eff[p.mat[,17] >= 0.5,17],res.afs.h$lpv[p.mat[,17] >= 0.5,17],xlab="alpha",main= "p >= 0.5")
plot.trend(p.mat[d.mat[,17] == -1 ,17],res.afs.h$lor[d.mat[,17] == - 1.0,17],xlab="p",main= "d == -1.0")
plot.trend(p.mat[d.mat[,17] == 0 ,17],res.afs.h$lor[d.mat[,17] == 0.0,17],xlab="p",main= "d == 0")
plot.trend(d.mat[p.mat[,17] >= 0.5,17],res.afs.h$lor[p.mat[,17] >= 0.5,17],xlab="d",main= "p >= 0.5", pch=17)
plot.trend(d.mat[p.mat[,17] >= 0.5,17],res.afs.h$lor[p.mat[,17] >= 0.5,17],xlab="d",main= "p >= 0.5")
plot.trend(d.mat[p.mat[,17] >= 0.5,17],res.afs.h$lor[p.mat[,17] >= 0.5,17],xlab="d",main= "p >= 0.5")
plot.trend((q.mat-p.mat)[d.mat[,17] == -1 ,17],res.afs.h$lor[d.mat[,17] == - 1.0,17],xlab="q-p",main= "d == -1.0")
plot.trend((q.mat-p.mat)[ ,17],res.afs.h$lor[,17],xlab="q-p",main= "d == -1.0")


plot(p.mat[d.mat[,17] == 0.0,17],res.afs.h$lor[d.mat[,17] == 0.0,17])
plot(p.mat[d.mat[,17] == -0.5,17],res.afs.h$lor[d.mat[,17] == -0.5,17])
plot(p.mat[d.mat[,17] == -1.0,17],res.afs.h$lor[d.mat[,17] == -1.0,17])
plot(p.mat[d.mat[,17] == -1.0,17],res.afs.h$lor[d.mat[,17] == -1.0,17])
abline(lm( res.afs.h$lor[d.mat[,17] == -1.0,17] ~ p.mat[d.mat[,17] == -1.0,17] ))
...
n.runs = 500
hs=c(0.0,0.25,0.5,0.75,1.0)
afs.vals=c(0.1,0.25,0.5)
lor.afs=list()

for (afs in afs.vals){
  res.tab = c()
  for( dom in hs) {
    res=run.simulations.rnd(n.runs=n.runs,h2=h2,total.ind =2100,n.replicates=3,n.pools=100,afs.vals=afs,h.vals=dom,F.vals=F.vals)
    res.tab=cbind(res.tab,log(res$or[,2]))
  }
  res.tab=as.data.frame(res.tab)
  colnames(res.tab) = hs
  lor.afs = append(lor.afs,list(res.tab))
}
names(lor.afs) = afs.vals

quartz(width=6,height=6)
cols=c("cyan","green","red")
bxwd=0.025
shift=0.0
boxplot(lor.afs[[1]],boxwex=bxwd,at=h.vals - 1 * (bxwd + shift) ,xlim=c(0- (bxwd + shift),1+ (bxwd + shift)), xaxt='n',col=cols[1],outline=F,ylab=expression(log(OR)), xlab ="h", add=F)
axis(1, at = h.vals)
mtext("Dominance",side=3,font=2,line=1.0)
boxplot(lor.afs[[2]],boxwex=bxwd,at=h.vals - 0 * (bxwd + shift), xaxt='n' ,yaxt='n' , col=cols[2],outline=F,add=T)
boxplot(lor.afs[[3]],boxwex=bxwd,at=h.vals + 1 * (bxwd + shift), xaxt='n' ,yaxt='n' , col=cols[3],outline=F,add=T)
legend("bottomleft",c("10%","25%","50%"),title="MAF",fill=c("cyan","green","red"))

dev.copy2pdf(file="dominance_lor.pdf")
...
wilcox.test(res.afs.h$p.lor,res.afs.h$p.pv,alternative = "greater")
...
#do abc for simulations
#load("two.loc.eu.sa_cb.RData")
#load("two.loc.eu.sa_cc.RData")
load("two.loc.eu.sa_d.RData")
# bring it into the right format
ats=attributes(two.loc.eps.eu)[["dimnames"]]
#ats$hs=ats$hs[-grep("1.75",ats$hs)]
# without afs
mastmat=matrix(nrow=0,ncol=8)
# with afs
#mastmat=matrix(nrow=0,ncol=16)
ats.hs = ats$hs
for (hsv in ats.hs){
  for (reffv in ats$rel.eff){
    for (epsv in ats$eps.int) { # c("0")){ #
        mat=matrix(data=rep(as.numeric(c(unlist(strsplit(hsv,":")),reffv,epsv)),length(ats$run))
                 ,nrow=length(ats$run),byrow=T)
      mat=cbind(mat,t(two.loc.eps.eu[hsv,reffv,epsv,"or",,]),t(two.loc.eps.sa[hsv,reffv,epsv,"or",,]))
      # for afs
      #mat=cbind(mat,t(two.loc.eps.eu[hsv,reffv,epsv,"or",,]),t(two.loc.eps.sa[hsv,reffv,epsv,"or",,]),
      #          t(two.loc.eps.eu[hsv,reffv,epsv,"af.l",,]),t(two.loc.eps.eu[hsv,reffv,epsv,"af.d",,]),
      #          t(two.loc.eps.sa[hsv,reffv,epsv,"af.l",,]),t(two.loc.eps.sa[hsv,reffv,epsv,"af.d",,]))
      mastmat=rbind(mastmat,mat)
    }  
  }
}
mastmat=as.data.frame(mastmat)
colnames(mastmat)=c("h1","h2","rel.eff","eps.int","or.eu.tan","or.eu.bab","or.sa.tan","or.sa.bab")
#colnames(mastmat)=c("h1","h2","rel.eff","eps.int","or.eu.tan","or.eu.bab","or.sa.tan","or.sa.bab","af.eu.l.tan","af.eu.l.bab","af.eu.d.tan","af.eu.d.bab","af.sa.l.tan","af.sa.l.bab","af.sa.d.tan","af.sa.d.bab")
mastmat[,c("or.eu.tan","or.eu.bab","or.sa.tan","or.sa.bab")]=log(mastmat[,c("or.eu.tan","or.eu.bab","or.sa.tan","or.sa.bab")])
str(mastmat)


criteria=c(2.51,0.57,1.89,0.44)
names(criteria)=c("eu","sa","tan_eusa","bab_eusa")
criteria.afs=c(0.43,0.07,0.88,0.74,0.39,0.14,0.75,0.24)
names(criteria.afs)=c("af.eu.l.tan","af.eu.d.tan","af.eu.l.bab","af.eu.d.bab","af.sa.l.tan","af.sa.d.tan","af.sa.l.bab","af.sa.d.bab")
# calculate criteria
for (i in c("or.eu.tan","or.eu.bab","or.sa.tan","or.sa.bab")){
  minim= min(mastmat[is.finite(mastmat[,i]),i])-1
  maxim= max(mastmat[is.finite(mastmat[,i]),i])+1
  mastmat[mastmat[,i] == Inf, i ] = maxim
  mastmat[mastmat[,i] == -Inf, i ] = minim
  #mastmat[mastmat[,i] == 0, i ] = 1e-16
}

mastmat$eu=mastmat$or.eu.tan/mastmat$or.eu.bab
mastmat$sa=mastmat$or.sa.tan/mastmat$or.sa.bab
mastmat$tan_eusa=mastmat$or.eu.tan/mastmat$or.sa.tan
mastmat$bab_eusa=mastmat$or.eu.bab/mastmat$or.sa.bab

# no eps: eps.int == 0:
mastmat.noeps = mastmat[mastmat$eps.int == 0,]
str(mastmat.noeps)

# do abc
tol = 0.1
method = "rejection"
rejection_samp =abc(target = criteria, param=mastmat[,c("h1","h2","rel.eff","eps.int")], sumstat = mastmat[,c("eu","sa","tan_eusa","bab_eusa")], tol = tol, method = method)
#quartz()
hist(rejection_samp)
...
tol = 0.1
method = "rejection"
#rejection.samp.noeps =abc(target = c(criteria,criteria.afs), param=mastmat.noeps[,c("h1","h2","rel.eff")], sumstat = mastmat.noeps[,c(names(criteria),names(criteria.afs))], tol = tol, method = method)
#quartz()
#hist(rejection.samp.noeps)

# do abc
tol = 0.1
method = "rejection"
rejection.samp.noeps =abc(target = c(criteria), param=mastmat.noeps[,c("h1","h2","rel.eff")], sumstat = mastmat.noeps[,c(names(criteria))], tol = tol, method = method)
#quartz()
hist(rejection.samp.noeps)

str(rejection.samp.noeps)
...
plot_acc_hist(rejection_samp,mastmat,criteria,cat.names=cat.names,filename="values_witheps")
plot_acc_hist(rejection.samp.noeps,mastmat.noeps,criteria,cat.names=cat.names,filename="values_noeps")
...
mdist.eps=get.mean.dists(rejection_samp,criteria)
mdist.noeps=get.mean.dists(rejection.samp.noeps,criteria)
...
dev.copy2pdf(file="mean_dist_comp.pdf")

# plot accepted and all values for abc
plot_acc_hist <- function(abc.obj,mastmat,criteria,cat.names=NULL,filename=FALSE){
  quartz()
  par(mfrow=c(2,2))
  ...
  names(cat.names)=names(criteria)
  for(i in names(criteria)){
    hist(abc.obj[["ss"]][,i],xlab="value",main=cat.names[i],breaks=c(-Inf,seq(-10,10,by=0.5),Inf),xlim=c(0,5),col=cols[1],) #,breaks=c(-Inf,seq(-10,10,by=0.5),Inf))
    hist(mastmat[,i],breaks=c(-Inf,seq(-10,10,by=0.5),Inf),col=cols[2],add=TRUE) 
    
    abline(v=criteria[i],col="red")
  }
  legend("topright", c("accepted", "all"), title="Simulations",fill=cols)
  if (filename != FALSE){
    dev.copy2pdf(file=paste0("abc_accpeted_rej_values",filename,".pdf"))
  }
  quartz()
  par(mfrow=c(2,2))
  for(i in names(criteria)){
    hist(abs(abc.obj[["ss"]][,i] -criteria[i]),xlab="distance",main=cat.names[i],breaks=c(seq(0,10,by=0.5),Inf),xlim=c(0,5),col=cols[1],) #,breaks=c(-Inf,seq(-10,10,by=0.5),Inf))
    hist(abs(mastmat[,i] - criteria[i]),breaks=c(seq(0,10,by=0.5),Inf),col=cols[2],add=TRUE) 
  }
  legend("topright", c("accepted", "all"), title="Simulations",fill=cols)
  if (filename != FALSE){
...
plot_hists <- function (abc.obj,ats,cols=c("h1","h2","rel.eff","eps.int"),
                        titles=c("dominance tan","dominance bab","effect of bab relative to tan","epistatic interaction strength"), 
                        fileprefix="abc_hist") {
  # plots separate histograms for each parameter
  # without eps: 
  # get ranges
  abc.obj=rejection_samp
  ranges=list()
  ranges$rel.eff=as.numeric(ats$rel.eff)
  ranges$eps.int=as.numeric(ats$eps.int)
  dom=apply(matrix(as.numeric(unlist(strsplit(ats$hs,":"))),ncol=2,byrow=T),2,function(x) unique(sort(x)))
  ranges$h1=dom[,1]
  ranges$h2=dom[,2]
  for (i in 1:length(cols)){
    valtab=table(abc.obj[[1]][,cols[i]])
    xvals=as.numeric(names(valtab))
    yvals=valtab/sum(valtab)
    xmin=min(ranges[[cols[i]]])
    xmax=max(ranges[[cols[i]]])
    barW=0.05*(xmax-xmin)
    #quartz()
    pdf(file=paste0(fileprefix,cols[i],".pdf"),width=4,height=4)
    plot(c(1,0.5),xlim=c(xmin-barW,xmax+barW),ylim=c(0,0.6),ylab="frequency",
      type="n",xaxt="n",main=titles[i])
    axis(1,at=xvals)
    rect(xleft=xvals-barW, ybottom=0, xright=xvals+barW, ytop=yvals, col=gray(0.5))
    dev.off()
  }   
}

plot_hists(rejection_samp,ats,cols=c("h1","h2","rel.eff","eps.int"))
plot_hists(rejection.samp.noeps,ats,cols=c("h1","h2","rel.eff"),fileprefix="abc_hist_noeps")

tol = 0.1
method = "loclinear"
loclin_samp = abc(target = criteria, param=mastmat[,c("h1","h2","rel.eff","eps.int")], sumstat = mastmat[,c("eu","sa","tan_eusa","bab_eusa")], tol = tol, method = method)
hist(loclin_samp)
#kills computer
tol = 0.1
method = "ridge"
#kills computer!!! dont do it mate!
#ridge_samp =abc(target = criteria, param=mastmat[,c("h1","h2","rel.eff","eps.int")], sumstat = mastmat[,c("eu","sa","tan_eusa","bab_eusa")], tol = tol, method = method)

summary(rejection_samp)

library(reshape2,ggplot2)
eff2=as.numeric(rev(ats$rel.eff))
# create objects with effects/epistatic interactions combinations and h1:h2
effxeps=expand.grid(rev(ats$rel.eff),ats$eps.int)
eff.noeps = rev(ats$rel.eff)
rownames(effxeps)=apply(effxeps,1,function (x) paste(x,collapse=":"))
colnames(effxeps)=c("rel.eff","eps.int")
hs2=matrix(as.numeric(unlist(strsplit(ats$hs,":"))),ncol=2,byrow=T)
rownames(hs2)=ats$hs
colnames(hs2)=c("h1","h2")
# create vectors of accepted combinations of h1:h2 and eff:eps for filling matrix
# with eps
# create matrix filled with zeros with rows: hs2 and columns: effxeps
rj.fq=matrix(0,nrow=length(ats$hs),ncol=length(rownames(effxeps)))
rownames(rj.fq)=ats$hs
colnames(rj.fq)=rownames(effxeps)
# fill matrix with number of occurences
for (i in 1:dim(rejection_samp[[1]])[1]) { 
  #print(i)
  rj.row=rejection_samp[[1]][i,]
  hs.str=paste(rj.row[1:2],collapse=":")
  effxeps.str=paste(rj.row[3:4],collapse=":")
  rj.fq[hs.str,effxeps.str] = rj.fq[hs.str,effxeps.str] + 1
}
# get frequency of each combination relative to maximal occurance:
rj.fq=rj.fq/5000

# no eps
eff.noeps = rev(ats$rel.eff)
# create matrix filled with zeros with rows: hs2 and columns: effxeps
rj.noeps.fq=matrix(0,nrow=length(ats$hs),ncol=length(eff.noeps))
rownames(rj.noeps.fq)=ats$hs
colnames(rj.noeps.fq)=eff.noeps
# fill matrix with number of occurences
for (i in 1:dim(rejection.samp.noeps[[1]])[1]) { 
  #print(i)
  rj.row=rejection.samp.noeps[[1]][i,]
  hs.str=paste(rj.row[1:2],collapse=":")
  eff.str=as.character(rj.row[3])
  rj.noeps.fq[hs.str,eff.str] = rj.noeps.fq[hs.str,eff.str] + 1
}
# get frequency of each combination relative to maximal occurance:
rj.noeps.fq=rj.noeps.fq/5000

library(reshape2,ggplot2)
source("/Volumes/vetgrid10/Tools/Scripts/R/image.scale.R")
plot.heatmap.fq <- function(fq.array,doms=c(0,0.25,0.5,0.75,1),breaknum=20, no.eps=FALSE,widths = c(7,1), heights = c(2,1), hs2=hs2, eff2= eff2, effxeps=effxeps,eps.int=eps.int)
{
  plot.dat=fq.array
  ...
  if (no.eps){
    ...
    axis(2,at=1:length(eff2),labels=abs(eff2))
    mtext(expression("effect of"~italic("bab1")~"relative to"~italic("tan")), side=2, line=2,cex=1.1)
  ...
  par(mar=iscale.par)
  image.scale(c(1.01,0.01), col=cols, breaks=sort(mybreaks),horiz=horizontal, yaxt="n",xaxt="n",xlab="",ylab="")  #print.breaks=c(0,-1,seq(-3,-24,by=-3))
#  axis(iscale.side,at=print.breaks,labels=print.breaks, las=2)
  axis(iscale.side,at=print.breaks,labels=print.breaks)
  mtext("Fraction", side=iscale.side, line=2.0,cex=1.1)
}
plot.dat=rj.fq
doms=unique(sort(unlist(hs2[,1])))
eff2=as.numeric(levels(effxeps[1:3,1]))
eps.int=as.numeric(levels(unique(sort(effxeps[,2]))))

save.image(file="thur16_07_2015.RSession")

quartz(width=10,height=8)
plot.heatmap.fq(rj.fq,doms=doms,breaknum=50)
dev.copy2pdf(file="heatmap_rej_method.pdf")

quartz(width=9,height=4.0)
plot.heatmap.fq(rj.noeps.fq,doms=doms,breaknum=50,no.eps=T,heights=c(4,2))
dev.copy2pdf(file="heatmap_rej_method_noeps.pdf")


### use uniformly distributed parameter values
load("simres.unif.eps.RData.gz")
uniform.sim.eps <- simulation_results
...
calculate_criteria <- function(unidf) {
  for (i in c("or.eu.tan","or.eu.bab","or.sa.tan","or.sa.bab")){
    ...
    unidf[unidf[,i] == Inf, i ] = maxim
    unidf[unidf[,i] == -Inf, i ] = minim
...
## simulations to compare with AJB

afs.1=c(0.25,0.5,0.75,1.0)
af.2=c(0.5)
eff=c(1,0)
hs.1=c(0.0,0.25,0.5,0.75,1.0)
h.2=c(0.5)
n.rep = 2 #number of replicates
total.ind =1000*n.rep # total individuals
n.runs=5
n.pools=100
h2=0.3
eps=0
F.inbreed=0.0
results <- data.frame(AF=as.numeric(),
                      h=as.numeric(),L=as.numeric(),M=as.numeric(),
                      D=as.numeric(),dL=as.numeric(),dD=as.numeric(),
                      aOR=as.numeric(),pOR=as.numeric())
for (h.1 in rev(hs.1)){
    for (af.1 in afs.1){
        n.freqs=c(af.1,af.2)    
      h.loci=c(h.1,h.2)
      v.E=get.V.E(n.freqs,eff,h.loci,h2,eps.int=eps)
      run.result=run_simulations.eps(n.runs,v.E,total.ind=total.ind,n.replicates=n.rep,n.pools=n.pools,eps.int=eps)
      result=c(mean(run.result[["af.l"]][,1]),mean(run.result[["af.m"]][,1]),mean(run.result[["af.d"]][,1]))
      results=rbind(results,c(af.1,h.1,result,result[1]-result[2],result[2]-result[3] ,mean(run.result[["or"]][,1])
                    ,result[1]*(1-result[3])/((1-result[1])*result[3]) )) 
    }
}
colnames(results)=c("AF","h","L","M","D","dL","dD","aOR","pOR")
View(results)
# results are very similar to Andreas
# check OR:
...
## create matrix of epistatic effects for comparison
### locus effects
loc.eff <- function(h.vals,g.type) { # takes the dominance and genotype vectors and returns a vector of gt effects (aa:1,aA:h,AA:0)
  return(g.type/2 + (g.type==1)*(h.vals-0.5))
}
...
tot.eff=1
eff.ratio=1
create.epimat <- function(h.loci=c(0.5,0.5),eff.ratio=1,eps.int=0.0,tot.eff=1.0,eff.epi=FALSE) {
  eff=c( 1/(1+eff.ratio) , eff.ratio/(1+eff.ratio) ) * tot.eff
  epi.mat=matrix(0,nrow=3,ncol=3,dimnames=list(c("aa","aA","AA"),c("bb","bB","BB")))
  for (i in 1:3) {
    for (j in 1:3){
      g.type=c(i,j)-1
      ind.eff <- loc.eff(h.loci,g.type)
      if (eff.epi){
        int <- ( eps.int*(ind.eff[1]-0.5)*(ind.eff[2]-0.5)) * mean(eff)
      }
      else{
        int <- ( eps.int*ind.eff[1]*ind.eff[2] - eps.int/2) * mean(eff)
      }
      epi.mat[i,j] <- eff %*% (ind.eff - 0.5 ) + int 
    }
  }
  attr(epi.mat,"eps.int") <- eps.int
  attr(epi.mat,"eff.ratio") <- eff.ratio
  attr(epi.mat,"h1") <- h.loci[1]
  attr(epi.mat,"h2") <- h.loci[2]
  return(epi.mat)
}
make_plot <- function(dat,add=F) {
  ats=unlist(attributes(dat))
  lty=2
  if (add == F){
  plot(c(0,1,2), c(min(dat), 0, max(dat)), xaxt="n",type="n", ylab="pheno", xlab="bab geno",main=paste("eps int=",ats["eps.int"]," eff.ratio =",ats["eff.ratio"]),ylim=c(-1,1)) 
  axis(side=1, at=c(0,1,2),labels=c("bb","Bb","BB"))
  lty=1
  }
  lines(x=c(0,1,2),y=dat[1,], col="blue", lwd = 2,lty=lty)
  text(label = "aa",x=1,y=dat[1,2], col="blue")
  lines(x=c(0,1,2),y=dat[2,], col="red", lwd = 2,lty=lty)
  text(label = "Aa",x=1,y=dat[2,2], col="red")  
  lines(x=c(0,1,2),y=dat[3,], col="green", lwd = 2,lty=lty)
  text(label = "AA",x=1,y=dat[3,2], col="green")
}
par(mfrow=c(1,3))
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=0.5,eff.ratio=1.0,eps.int=-1.0))
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=0.5,eff.ratio=1.0,eps.int=0.0))
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=0.5,eff.ratio=1.0,eps.int=1.0))

make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=1.0,eff.ratio=1.0,eps.int=-1.0))
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=1.0,eff.ratio=1.0,eps.int=-1.0,eff.epi=T),add=T)
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=1.0,eff.ratio=1.0,eps.int=0.0))
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=1.0,eff.ratio=1.0,eps.int=0.0,eff.epi=T),add=T)
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=1.0,eff.ratio=1.0,eps.int=1.0))
make_plot(create.epimat(h.loci=c(0.5,0.5),tot.eff=1.0,eff.ratio=1.0,eps.int=1.0,eff.epi=T),add=T)

get.Vs <- function(afs_loc,eff.ratio,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=0.0,tot.eff=1.0){
  gt.freqs = sapply(afs_loc,function (p) c(p^2 + (p-p^2)*F.in,2*(p-p^2)*(1-F.in),(1-p)^2 + (p-p^2)*F.in))
  pos <- c()
  pos <- apply(gt.freqs,2,function (x) get.gt.locus(x,c(2,1,0),n))
  pos <- matrix(pos,nrow=length(afs_loc),ncol=n,byrow=T)
  ...
  ind.eff <- apply(pos,2,function(x) loc.eff(h_loc,x)) 
  ## interaction component
  intxn <- ( eps.int*ind.eff[1,]*ind.eff[2,] - eps.int/2.0  )*tot.eff
  ## additive effects (eff centered around 0)
  pheno <- eff %*% ( ind.eff - 0.5 )
  v.A=var(as.vector(pheno))
  v.I=var(intxn)
  pheno <- pheno + intxn
  v.P= var(as.vector(pheno))
  v.E=(1-h2)/h2*var(as.vector(pheno))
  Vs=c(eps.int,eff.ratio,v.A,v.I,v.P,v.E)
  names(Vs)=c("eps.int","eff.ratio","v.A","v.I","v.P","v.E")
  return(Vs)
}

variances=matrix(,nrow=0,ncol=6)
colnames(variances)=c("eps.int","eff.ratio","v.A","v.I","v.P","v.E")
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=1,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=0.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=2,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=0.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=3,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=0.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=1,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=2,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=3,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=1,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=-1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=2,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=-1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.eu,eff.ratio=3,h_loc=c(0.5,0.5),h2=0.3,F.in=0,n=1000,eps.int=-1.0,tot.eff=1.0))
...
colnames(variances)=c("eps.int","eff.ratio","v.A","v.I","v.P","v.E")
h.loci=c(1.0,0.0)
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=1,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=0.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=2,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=0.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=3,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=0.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=1,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=2,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=3,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=1,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=-1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=2,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=-1.0,tot.eff=1.0))
variances=rbind(variances,get.Vs(afs.sa,eff.ratio=3,h_loc=h.loci,h2=0.3,F.in=0,n=1000,eps.int=-1.0,tot.eff=1.0))
