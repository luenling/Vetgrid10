...
def mapping_reads(arg_dict,simchr):
    ...
    gsnap_command = "gsnap -d "+ gsnap_dbname +" -D "+ gsnap_dbdir +" "+read1+" "+read2+" --no-sam-headers --split-output="+str(output_name)+" --pairmax-dna="+str(int(maximum_fragment_size))+" --pairexpect="+str(int(mean_fragment_size))+" --quality-protocol="+fastqtype+" -t "+str(int(thread))+" -A sam -B 5"
...
def read_sam_ambigious(input,simchr):
    ...
    for line in fh:
        if line:
            ...
            if headerm==None:
                ...
                read_id = a[0].rstrip().lstrip()
                contig_id = a[2].rstrip().lstrip()
                mapping_quality = a[4].rstrip().lstrip()
                sim_reads[read_id]="na"
    ...
    for line in fh:
        if line:
            ...
            if headerm==None:
                ...
                contig_id = a[2].rstrip().lstrip()
                mapping_quality = a[4].rstrip().lstrip()
		value = str(contig_id)+"#"+str(mapping_quality)
		sim_reads[read_id] += "\t"+str(value)
		    
    
    fh.close()
    
    for k,v in sim_reads.iteritems():
	map_list = v.split("\t")
	map_list.pop(0)
	each_read_dict = {}
	for r in map_list:
	    contig,mapping_quality = r.split("#")
	    each_read_dict[r] = int(mapping_quality)
	
	sorted_each_read_dict = sorted(each_read_dict.iteritems(), key=operator.itemgetter(1),reverse=True)
	
	
	contig_id,mapping_quality1 = sorted_each_read_dict[0][0].split("#")
	
	
	### checking if mel and sim both have same mapping quality
	max_mq = int(mapping_quality1)
	
	contigs_list = []
	for r in sorted_each_read_dict:
	    contig_id2,mapping_quality2 = r[0].split("#")
	    if int(mapping_quality2)>=max_mq:
		contigs_list.append(contig_id2)
	...
	if len(contigs_list)<=sim_contig_count:
	    sim_reads_final[k]=contig_id
