def read_snp_file(infile):
    """
    reads a sync like file with majmin alleles pV odds ratios and intervals and creates a chrom->bps->[alleles,[ afs...]]
    """
...
import argparse
parser = argparse.ArgumentParser(description='reads pos from file A and only gets the lines with the same positions from file B and changes the maj minor alleles and afs accordingly to file B. writes B + A to stdout or only the changed A if -oa ') 
...
parser.add_argument("--oa", dest="oa",action='store_true', help="only change allele order and afs in file a and print it", default=False)
...
for line in inf:
    ...
    if ( fields[0] in snp_dict and fields[1] in snp_dict[ fields[0] ]):
        if snp_dict[ fields[0] ][ fields[1] ][0] == fields[2][::-1]:
            snp_dict[ fields[0] ][ fields[1] ][1] = 1.0 - snp_dict[ fields[0] ][ fields[1] ][1]
