...
F=1
awk 'BEGIN {OFS = "\t"} {key=sprintf("%-25s%09d",tolower($1),$2); gsub(/ /,"-",key); print key,$0}' $INFA > ${INFA}".temp"
awk 'BEGIN {OFS = "\t"} {key=sprintf("%-25s%09d",tolower($1),$2); gsub(/ /,"-",key); print key,$0}' $INFB > ${INFB}".temp"
